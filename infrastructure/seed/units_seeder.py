...
class UnitsSeeder:
    ...
    async def Seed(self, seed_file: str|None = None) -> bool:
        ...
        existing = await UnitRepository().get_all(self.server_id)
        if existing:
            return False
        ...
        for u in unit_data["data"]:
            ...
            success, _ = await UnitRepository().add(unit)
            if not success:
                has_failures = True
...
class UnitStatsSeeder:
    ...
    async def Seed(self, seed_file: str|None = None) -> bool:
        ...
        for stat in unit_stat_data["data"]:
            unit = await UnitRepository().get_by_name(stat["unit_name"], self.server_id)
            if not unit:
            ...
            success, _ = await UnitStatRepository().add(unit_stat)
            if not success:
                has_failures = True
