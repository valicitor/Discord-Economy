from domain import Vehicle, IRepository
from infrastructure import BaseRepository
from typing import List, Optional

class VehicleRepository(BaseRepository, IRepository):
    
    # ---------- Schema Setup ----------

    async def init_database(self):
        """
        initalizes the database schema for the actions table. Called automatically on first use. Override in child classes to create tables.
        connection is managed by BaseRepository, so we can use super() to execute our schema setup queries.
        """
        conn = await super().acquire_connection()

        await conn.execute("""
            CREATE TABLE IF NOT EXISTS vehicles (
                vehicle_id INTEGER PRIMARY KEY AUTOINCREMENT,
                server_id INTEGER NOT NULL,
                name TEXT NOT NULL,
                description TEXT DEFAULT '',
                metadata TEXT,
                FOREIGN KEY(server_id) REFERENCES servers(server_id),
                UNIQUE(name, server_id)
            )
        """)

        await conn.commit()
    
    # ---------- Teardown ----------

    async def drop_table(self):
        """
        Drops the vehicles table. Use with caution! This will delete all data in the table and cannot be undone.
        """
        await super().execute(f"DROP TABLE IF EXISTS vehicles")

    async def clear_all(self) -> bool:
        """
        Clears all data from the vehicles table. Use with caution! This will delete all data in the table and cannot be undone.
        """
        affected = await super().delete(
            "DELETE FROM vehicles"
        )
        await super().execute("DELETE FROM sqlite_sequence WHERE name = ?", "vehicles")
        return affected > 0

    # ---------- Queries ----------

    async def get_by_id(self, vehicle_id: int) -> Optional[Vehicle]:
        row = await super().fetchrow(
            "SELECT * FROM vehicles WHERE vehicle_id = ?",
            vehicle_id
        )
        return Vehicle(data=dict(row)) if row else None

    async def get_all(self, server_id: int) -> List[Vehicle]:
        rows = await super().fetch("SELECT * FROM vehicles WHERE server_id = ?", server_id)
        return [Vehicle(data=dict(row)) for row in rows]
    
    # ---------- Additional Queries ----------

    async def get_by_name(self, name: str, server_id: int) -> Optional[Vehicle]:
        row = await super().fetchrow(
            "SELECT * FROM vehicles WHERE name = ? AND server_id = ?", 
            name, 
            server_id
        )
        return Vehicle(data=dict(row)) if row else None
    
    # ---------- Existence Checks ----------

    async def exists(self, vehicle_id: int) -> bool:
        row = await super().fetchrow(
            "SELECT 1 FROM vehicles WHERE vehicle_id = ?",
            vehicle_id
        )
        return row is not None
    
    # ---------- Additional Existence Checks ----------

    # ---------- Mutations ----------

    async def insert(self, vehicle: Vehicle) -> int:
        return await super().insert(
            "INSERT INTO vehicles (server_id, name, description, metadata) VALUES (?, ?, ?, ?)",
            vehicle.server_id,
            vehicle.name,
            vehicle.description,
            str(vehicle.metadata)
        )
    
    async def update(self, vehicle: Vehicle) -> bool:
        affected = await super().update(
            "UPDATE vehicles SET server_id = ?, name = ?, description = ?, metadata = ? WHERE vehicle_id = ?",
            vehicle.server_id,
            vehicle.name,
            vehicle.description,
            str(vehicle.metadata),
            vehicle.vehicle_id
        )
        return affected > 0

    async def delete(self, vehicle: Vehicle) -> bool:
        affected = await super().delete(
            "DELETE FROM vehicles WHERE vehicle_id = ?",
            vehicle.vehicle_id
        )
        return affected > 0
    
    async def delete_all(self, server_id: int) -> bool:
        # Delete_all and clear_all do the same in this repository since there are no environment variables to restrict by.
        affected = await super().delete(
            "DELETE FROM vehicles WHERE server_id = ?",
            server_id
        )
        return affected > 0