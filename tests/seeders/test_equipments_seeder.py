...
import unittest
from infrastructure import EquipmentRepository, EquipmentStatRepository, SeedEquipmentsAndEquipmentStatsIfEmpty

class TestEquipmentsSeeder(unittest.TestCase):
    def setUp(self):
        self.equipment_repository = EquipmentRepository(db_path=":memory:")  # Use in-memory database for testing
        self.equipment_stat_repository = EquipmentStatRepository(db_path=":memory:")  # Use in-memory database for testing
    ...
    def test_seed_equipments_and_equipment_stats_if_empty(self):
        # Act
        SeedEquipmentsAndEquipmentStatsIfEmpty(self.equipment_repository, self.equipment_stat_repository)
